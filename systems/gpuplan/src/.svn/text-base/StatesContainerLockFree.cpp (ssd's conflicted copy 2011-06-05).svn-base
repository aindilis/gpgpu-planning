...
StatesContainerLockFree::StatesContainerLockFree(myModel * newModel) {
	...
	int size = 0;
	if (model->initial->size < 156 ) size = 27;
	if (model->initial->size < 76 ) size = 28;
	if (model->initial->size < 33 ) size = 29;
	if (model->initial->size < 16 ) size = 30;
	assert(size);

	dbs = DBSLLcreate(model->initial->size,hashLength,size);
	open = new openList(model->initial->size);
...
bool StatesContainerLockFree::insert(unsigned int bfslayer, myState * state){
	int idx;
	bool seen;
	
	unsigned int newLayer = state->cost();
	if(bfslayer) assert(newLayer);
	seen =  lookup_ret(dbs,(const int*) state->vector,&idx);
