...
*/
extern dbs_ll_t     DBSLLcreate (int len, int hashLength,int size);
extern dbs_ll_t     DBSLLcreate_sized (int len, int size, hash32_f hash32,int hashLength);
...
extern int          DBSLLlookup_hash (const dbs_ll_t dbs, const int *v,
                                      int *ret, uint32_t * hh,char insert);

extern char         *DBSLLget (const dbs_ll_t dbs, const int idx, int *dst);
